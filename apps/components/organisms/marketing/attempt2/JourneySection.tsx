      ...
      <div class="max-w-7xl mx-auto space-y-16 text-center">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 text-left">
          ...
          <div class="bg-neutral-900 border border-white/10 rounded-xl p-8 space-y-6 shadow-inner">
            <h3 class={`text-xl font-semibold ${beforeStyles.text}`}>Before Open Industrial</h3>
            <div class="space-y-4 text-sm">
              <p class={`text-neutral-400 border-l-4 pl-4 ${beforeStyles.border}`}>
                Needed a multi-year MSA to add a rule
              </p>
              <p class={`text-neutral-400 border-l-4 pl-4 ${beforeStyles.border}`}>
                Data locked behind SIs or aging APIs
              </p>
              <p class={`text-neutral-400 border-l-4 pl-4 ${beforeStyles.border}`}>
                Every logic update risked a production outage
              </p>
              <p class={`text-neutral-400 border-l-4 pl-4 ${beforeStyles.border}`}>
                No clear version history of control logic
              </p>
              <p class={`text-neutral-400 border-l-4 pl-4 ${beforeStyles.border}`}>
                Dashboards took quarters, not minutes
              </p>
            </div>
            <FigureImage
              src="/assets/before-legacy-stack.png"
              alt="Legacy industrial software stack before Open Industrial"
              caption="Legacy vendor lock-in and brittle integrations."
