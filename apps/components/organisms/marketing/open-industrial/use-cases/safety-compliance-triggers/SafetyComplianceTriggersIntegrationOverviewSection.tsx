...
import { safetyComplianceTriggersIntegrationSteps } from '../../../../../../../src/marketing/use-case/safety-compliance-triggers.ts';
...
export default function SafetyComplianceTriggersIntegrationOverviewSection(): JSX.Element {
  return (
      ...
      <div class='space-y-12 text-neutral-700 dark:text-neutral-200'>
        <div class='mx-auto max-w-3xl space-y-4 text-center'>
          ...
          <h2 class='text-balance text-3xl font-semibold tracking-tight text-neutral-900 sm:text-4xl dark:text-white'>
            Connect Safety and Compliance Signals in One Hub
          </h2>
          <p class='text-base leading-relaxed text-neutral-700 sm:text-lg dark:text-neutral-300'>
            Open Industrial ingests telemetry, control, and compliance data to automate trigger
            detection and logging without manual effort.
          </p>
        </div>

        <ol class='mx-auto grid w-full max-w-4xl gap-4 text-left sm:grid-cols-2'>
          {safetyComplianceTriggersIntegrationSteps.map((step, index) => (
            <li
              key={step}
              class='relative overflow-hidden rounded-3xl border border-neutral-200/70 bg-white/90 px-5 py-5 shadow-[0_38px_140px_-110px_rgba(236,72,153,0.3)] transition-transform duration-200 hover:-translate-y-1 dark:border-white/10 dark:bg-neutral-900/80 dark:shadow-[0_55px_180px_-130px_rgba(236,72,153,0.45)]'
            >
              <div
              ...
              <div class='flex items-start gap-4'>
                <span class='mt-1 inline-flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br from-neon-pink-500/20 via-neon-purple-500/20 to-neon-blue-500/20 text-sm font-semibold text-neutral-700 dark:text-neutral-200'>
                  {index + 1}
                </span>
