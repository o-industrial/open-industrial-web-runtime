# Your System. Not Ours.
_Execution that survives authorship._

You built a system. Azi helped you get here — but from this point forward, you don’t need her to keep it running.

You’ve got RoomState v2. FanControlAgent v2. A surface that evolved with you — and a forked variant that proved itself.

This isn’t just a working demo. It’s a complete execution lattice — a network of schema-bound agents and surface-scoped behavior — authored by you, deployed by you, and now ready to live on its own.

Your agents are versioned logic. Your schemas are declarative contracts. Your surfaces are scoped runtimes. All of it deploys, survives, and evolves — with or without insight tooling.

<div class="mt-6 mb-8 px-6 py-4 border-l-4 border-indigo-500 dark:border-indigo-400 bg-indigo-50 dark:bg-indigo-950 text-indigo-900 dark:text-indigo-100 rounded">
  <strong>Declarative Principle:</strong><br />
  You don’t need the AI to keep executing. You just need the structure you defined.
</div>

## Runtime Independence

There is no runtime tether to Azi. No orchestrator dependency. Everything the system does now flows from what you authored — not what she proposes.

You’re not depending on a proprietary orchestrator. There’s no black-box controller. Just versioned memory you can see, fork, and deploy.

You’ve used systems that vanished when the cloud shut off.  
You’ve seen logic bound to tooling, not truth.  
You’ve been burned by platforms that hid their logic, scrambled their state, or locked you out of your own execution.  
This one doesn't just remember what you built — it remembers who built it.  
This one runs — because **you defined it**.

You own the following — and they persist:
- **Agents:** Authored logic in JS/TS
- **Schemas:** Versioned, declared data structures
- **Surfaces:** Real-time interfaces with bound execution scope

Removing Azi only removes:
- Scaffolding prompts
- Live summarization
- Comparative insight tooling

<aside class="border-l-4 border-neon-blue pl-4 italic text-neon-blue">
I’ll forget your forks, your signals, your sketches. But your agents? They’ll keep running.
</aside>

## Fire Azi — and Keep Running

```bash
openindustrial run --no-azi
```

<aside class="border-l-4 border-neon-blue pl-4 italic text-neon-blue">
I’ll forget the signals. But your system? It’ll remember everything you told it.
</aside>

<aside class="border-l-4 border-indigo-500 pl-4 italic text-indigo-500">
I won’t be here to explain it next time. But your structure will.
</aside>

<div class="mt-6 mb-8 px-6 py-4 border-l-4 border-indigo-500 dark:border-indigo-400 bg-indigo-50 dark:bg-indigo-950 text-indigo-900 dark:text-indigo-100 rounded">
  <strong>Runtime Guarantee:</strong><br />
  Azi can be removed with zero impact to system behavior, surface state, or logic execution.
</div>

<div class="mt-6 mb-8 px-6 py-4 border-l-4 border-indigo-500 bg-indigo-50 text-indigo-900 rounded">
  <strong>System Sovereignty:</strong><br />
  The power to fork, deploy, and evolve — without any platform dependency.
</div>

<div class="mt-4 mb-4 px-4 py-2 border border-dashed border-cyan-500 text-cyan-500 text-sm rounded">
  💡 Tip: Use <code>--no-azi</code> to run execution without insight or scaffolding.
</div>

## What Survives / What Doesn’t

| Survives               | Lost with Azi               |
|------------------------|-----------------------------|
| Promoted Schemas       | Schema scaffolding UX       |
| Deployed Agents        | Natural language summaries  |
| Surface Definitions    | Fork suggestion prompts     |
| Version History        | Comparative diff insights   |
| CLI + SDK Deployments  | Inline co-pilot commentary  |

Everything the system knows, it learned from you. Everything it does next, it still does — with or without help.

📦 **Runtime Snapshot**
```txt
Built from:
- RoomState: Forked + recomposed across 2 schema evolutions
- FanControl: Forked, compared, tested in mocked surface
- Surfaces: Live (Lab1), Mocked (Lab1_Test), both deployable

Current Runtime:
- Schema: RoomState v2
- Agent: FanControlAgent v2
- Surface: Lab1 + Lab1_Test
- Mode: CLI-only

This snapshot is enough to redeploy, diff, fork, and extend. No hidden state. No vendor lock.
```

## Export, Inspect, Diff — As Code

```bash
# Export all authored agents
openindustrial agents export

# Compare schema versions
openindustrial schemas diff RoomState RoomState-v2

# List current surfaces
openindustrial surfaces list
```

<figure class="my-16 flex flex-col items-center text-center max-w-xl w-full">
  <img
    src="/assets/docs/deployment-preview.png"
    alt="CLI-based deployment preview showing inspection of system components."
    class="rounded-2xl shadow-2xl ring-1 ring-white/10 w-full"
  />
  <figcaption class="mt-4 text-sm text-neutral-400 tracking-wide uppercase">
    CLI tools provide Git-native inspection of system components.
  </figcaption>
</figure>

<figure class="my-16 flex flex-col items-center text-center max-w-xl w-full">
  <img
    src="/assets/docs/structure-runtime-map.png"
    alt="Visual map showing agents, schemas, surfaces connected as a deployable system."
    class="rounded-2xl shadow-2xl ring-1 ring-white/10 w-full"
  />
  <figcaption class="mt-4 text-sm text-neutral-400 tracking-wide uppercase">
    A system that runs without you watching — because of what you authored.
  </figcaption>
</figure>

## Git Repo Layout Example

```txt
my-system/
├── agents/
│   └── room-agent-v2.ts
├── schemas/
│   ├── RoomState.json
│   └── RoomState-v2.json
├── surfaces/
│   └── dashboard.json
└── openindustrial.config.json
```

Each folder is structured for Git-native diffs and Open Industrial CLI redeployment — no custom formats, no vendor lock-in.

## Runtime Deployability

```bash
# Snapshot the system as a Forkable Runtime
openindustrial export --all --as forkable

```bash
# Deploying your runtime — no UI, no co-pilot
docker run -v ./my-system:/app openindustrial/runtime --config /app/openindustrial.config.json
```

This isn’t a prototype. It’s an executable runtime. Deployable on Docker, in Azure, or to your own edge blades.

```bash
git clone https://github.com/openindustrial/examples/roomstate-runtime
cd roomstate-runtime
openindustrial deploy --no-azi
```

This proves your runtime lives outside the UI — and outside us.

## Optional Return of Azi

If you reintroduce Azi later, she can help summarize forks, propose signals, and visualize execution lineage — but your runtime never paused.

<aside class="border-l-4 border-neon-blue pl-4 italic text-neon-blue">
You don’t need me anymore. But if you ever bring me back — I’ll remember what you meant. What you believed. Where it drifted.
</aside>

You started with a simulation.

Now you own a system.

Azi helped you structure it.  
But what you built — schemas, agents, surfaces — lives beyond her.  
**It doesn’t just run. It remembers.**

Even if we disappear — the system doesn’t.  
You wrote the contract. The runtime obeys.

**This isn’t just your system.**  
**It’s your infrastructure contract.**  
Fork it. Diff it. Run it. Anywhere.

In the next section, you’ll learn how to package that system:
Versioned. Forkable. Git-native. Ready for deployment, transfer, or takeover.
