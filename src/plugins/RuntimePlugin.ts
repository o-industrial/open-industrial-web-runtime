...
export default class RuntimePlugin implements EaCRuntimePlugin {
  ...
  public Setup(config: EaCRuntimeConfig) {
    ...
    console.log("Running in Docker: ", isInDockerCgroupSync());
...
function isInDockerCgroupSync(): boolean {
  try {
    const data = Deno.readTextFileSync("/proc/1/cgroup");
    return /docker|kubepods/.test(data);
  } catch {
    return false;
  }
}
